...
static time_t requestTime;
static int limitTime = 4;


void updateWeightThread(ngx_http_upstream_rr_peers_t * peers) {
    ngx_http_upstream_rr_peers_wlock(peers);
    ...
    // int tmp = 0;
    // for(int i = 0;i<2;i++){
    //     peer->weight =(ngx_int_t) tmp;
    //     peer->effective_weight = (ngx_int_t)tmp;
    //     peer->current_weight = (ngx_int_t)tmp;
    //     peers->total_weight += (ngx_uint_t)tmp;
    ...
    char weight[10];
    int point = 0;
    if((fp = fopen("/home/zn/Desktop/weight.txt","rb+"))  == NULL) {

        FILE * tmpFile;
        tmpFile = fopen("/home/zn/Desktop/tmp.txt","wb+");
        fprintf(fp,"hello\n");
        fclose(tmpfile);
    ...
    while((c = fgetc(fp)) != EOF) {
        if(c == '\n') {
            peer->weight = (ngx_int_t)atoi(weight);
            peer->effective_weight = (ngx_int_t)atoi(weight);
            peer->current_weight = (ngx_int_t)atoi(weight);
            peers->total_weight += (ngx_uint_t)atoi(weight);
            memset(weight, 0, sizeof(weight));
            point = 0;
            peer = peer->next;
            continue;
...
{
    //-------------weijie------------
    time_t nowTime = ngx_time();
    if(isChanging == 0 && nowTime - requestTime > limitTime)
    {
        isChanging = 1;
        updateWeightThread(rrp->peers);
        requestTime = nowTime;
    }

    //-------------zm---------------
    // ngx_http_upstream_rr_peers_wlock(rrp->peers);
    // rrp->peers->total_weight = 0;
    // ngx_http_upstream_rr_peer_t * peer1;
    // peer1 = rrp->peers->peer;

    // FILE * tmpFile;
    // tmpFile = fopen("/home/zn/Desktop/tmp.txt","wb+");
    // int tmp = 0;
    // for(int i = 0;i<2;i++){
    //     peer1->weight =(ngx_int_t) tmp;
    //     peer1->effective_weight = (ngx_int_t)tmp;
    //     peer1->current_weight = (ngx_int_t)tmp; 
    //     fprintf(tmpFile,"%d\n",peer1->current_weight);
    
    //     rrp->peers->total_weight += (ngx_uint_t)tmp;
    //     //memset(weight, 0, sizeof(weight));
    //     //point = 0;
    //     peer1 = peer1->next;
    //     // tmp++;
